...
namespace QuantLib {
    ...
    Gsr::Gsr(Handle<YieldTermStructure> termStructure,
             std::vector<Date> volstepdates,
             const std::vector<Real>& volatilities,
             const Real reversion,
             const Real T)
    : Gaussian1dModel(std::move(termStructure)), CalibratedModel(2), reversion_(arguments_[0]),
      sigma_(arguments_[1]), volstepdates_(std::move(volstepdates)) {

        QL_REQUIRE(!this->termStructure().empty(), "yield term structure handle is empty");
